...
function validaClick(ambienteSelecionado) {
    ...
    if (ambienteSelecionado == "GAUSS") {
        ...
        urlGeraToken = "http://192.168.0.201:8093/rest/api/oauth2/v1/token?grant_type=password&password=fenixgauss@06&username=admin"
        geraToken(urlGeraToken, url);

    } else if (ambienteSelecionado == "CDG") {
        url = "http://179.108.122.43:8090/rest/stocksg"
        urlGeraToken = "http://179.108.122.43:8090/rest/api/oauth2/v1/token?grant_type=password&password=fenixcdg@06&username=admin "
        console.log(`Ambiente selecionado é ${ambienteSelecionado}`);
        geraToken(urlGeraToken, url);


    } else if (ambienteSelecionado == "SPG") {
        url = "http://179.108.122.43:8096/rest/stocksg"
